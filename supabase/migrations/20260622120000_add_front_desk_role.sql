...
ALTER TABLE public.user_gym_roles
ADD CONSTRAINT user_gym_roles_role_check CHECK (
    role IN (
      'super_admin',
      'gym_owner',
      'gym_admin',
...
CREATE POLICY "gym_staff_checkins" ON public.member_check_ins FOR
SELECT USING (
    EXISTS (
      SELECT 1
...
CREATE POLICY "gym_staff_memberships" ON public.gym_memberships FOR ALL USING (
  EXISTS (
    SELECT 1
    FROM public.user_gym_roles ugr
    WHERE ugr.user_id = auth.uid()
      AND (
        ugr.gym_id = gym_memberships.gym_id
        OR ugr.role = 'super_admin'
      )
      AND ugr.role IN (
        'super_admin',
        'gym_owner',
