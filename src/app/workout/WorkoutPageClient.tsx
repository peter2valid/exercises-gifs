...
function ActiveView({
  ...
  useEffect(() => {
    ...
    if (preferredId) {
      setExerciseId((current) => current || preferredId);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
      ...
      <div className="max-w-md mx-auto w-full">
        ...
        <div className="px-4 pt-6 pb-6">
              ...
              <div className="space-y-2 max-h-40 overflow-y-auto scrollbar-hide">
                {[...sets].reverse().slice(0, 4).map((s) => (
                  <div key={s.id} className="glass-panel p-3 flex items-center justify-between">
                    <span className="text-xs text-white/50">{exMap[s.exercise_id]?.name ?? s.exercise_id}</span>
          ...
          <div className="glass-panel p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Button onClick={onStartRest} disabled={!hasLastSet} className="h-10 text-sm" variant="secondary">
                <Pause size={16} className="mr-1" />
                Rest
              </Button>
              <Button onClick={onComplete} disabled={sets.length === 0} className="h-10 text-sm" variant="secondary">
        ...
        <div className="animate-fade-in">
          ...
          {lastSet && (
            <div className="glass-panel p-6">
              ...
              <p className="text-white/60 text-sm mb-2">{exMap[lastSet.exercise_id]?.name ?? lastSet.exercise_id}</p>
